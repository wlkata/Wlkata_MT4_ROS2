...
本工程是一个用于 **WLKATA MT4 系列机械臂** 的综合控制与示教工程，主要包含：

- **机械臂模型与描述**（`wlkata_mt4_description`）：URDF/Xacro、关节配置等，用于在 RViz / 仿真中显示机器人。
- **MoveIt 运动规划配置**（`wlkata_mt4_moveit_config`）：基于 MoveIt 2 的配置包，用于对 MT4 进行路径规划与可视化。
...
- 使用 ROS 2 + MoveIt 进行 **MT4 运动规划与可视化**；
...
- `wlkata_mt4_description/`
  - `urdf/`：`wlkata_mt4_description.urdf`、`.xacro` 等机械臂模型文件。
  - `config/`：关节名称、配置等 YAML 文件。
  - `launch/display.launch.py`：在 RViz 中显示 MT4 模型。

- `wlkata_mt4_moveit_config/`
  - `config/`：MoveIt 所需的 `kinematics.yaml`、`ros2_controllers.yaml`、`wlkata_mt4_description.srdf` 等。
  - `launch/`：
...
- `wlkata_arm_move/`
  - `wlkata_arm_move/mt4_moveit_move.py`：核心 Python 节点，订阅 `/display_planned_path`（`moveit_msgs/DisplayTrajectory`）话题，将规划出的关节轨迹转换为 G-code 命令，经串口发送给机械臂。
  - `package.xml`：ROS 2 Python 包定义，依赖 `rclpy`、`moveit_msgs`、`wlkata_interfaces`、`action_msgs`、`std_msgs` 等。
  - `setup.py`：通过 `entry_points` 暴露 `mt4_moveit_move` 可执行节点。

- `wlkatapython/`
  - `wlkatapython.py` 及多个子目录（`Mirobot_robot/`、`E4_robot/`、`MT4_robot/`、`MS4220_robot/`、`Harobot_robot/` 等）：各类设备的串口控制实现。
  - `README.md`：详细介绍了如何通过 Python + 串口 + G-code 控制 MT4/E4/MS4220 等设备（含示例代码与接线图）。
...
- `ros2 launch wlkata_mt4_description display.launch.py`：只启动机器人描述与 RViz，可看到 **MT4 模型**、关节状态发布/GUI，主要用于检查 URDF、关节配置是否正确，不包含 MoveIt 规划界面。
- `ros2 launch wlkata_mt4_moveit_config demo.launch.py`：启动 **MoveIt + RViz 演示场景**，在 RViz 左侧会出现 MotionPlanning 面板，可以交互规划轨迹并执行（仿真或配合真实机械臂节点）。
...
在 `mt4_moveit_move.py` 中，默认串口设备为：
...
- 提供 mt4 机械臂的模型描述（URDF/Xacro）、关节配置与显示/仿真所需的 Launch 文件，可用于检查模型与坐标系是否正确。
...
- ROS 2 Python 包，核心节点 `mt4_moveit_move`：接收 MoveIt 规划出的轨迹，将其转换为符合 WLKATA 控制器的 G-code，通过串口发送给实际机械臂，并处理返回信息。

### `wlkatapython`

- 独立 Python 包，可单独 `pip install` 使用，支持 mt4、E4、MT4、MS4220 等多种设备，支持 RS485 与 UART 通信；其 `README.md` 中包含大量示例与接线图，本工程中的 ROS 2 控制节点直接复用了其中的通信逻辑。
