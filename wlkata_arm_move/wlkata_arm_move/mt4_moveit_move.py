...
class TrajectorySubscriber(Node):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.mt4=MT4_UART(True,False)
            self.mt4.init(self.ser,-1)
    ...
    def listen_trajectory(self, msg):
        ...
        for point in traj.points:
            if len(point.positions) < 4:
                self.get_logger().warn("Skipping point with fewer than 4 joints.")
                continue

            # Convert radians to degrees
            degs = [round((p / math.pi) * 180, 2) for p in point.positions[:4]]
            j1, j2, j3, j4= degs

            # Format G-code with spaces and newline
            command = f"M21 G90 G00 X{j1} Y{j2} Z{j3} A{j4}\n"
